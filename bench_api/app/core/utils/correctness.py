    ...
    with torch.no_grad():
        for _ in range(config.NUM_CORRECT_TRIALS):
            ...
            if ref_output.shape != new_output.shape:
                return False, f"Output shape mismatch: Expected {ref_output.shape}, got {new_output.shape}"
            ...
            if not torch.allclose(ref_output, new_output, **backend.tolerances):
                ...
                return False, f"Output mismatch: max_diff={max_diff:.6e}, mean_diff={mean_diff:.6e}, match_rate={match_rate:.2f}%"
