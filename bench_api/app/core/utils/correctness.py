...
def run_correctness(backend, ref_src: str):
    ...
    """
    # Only exec ref_src if Model is not already in context.
    # When validation pre-loads Model before compile, re-execing here would
    # create a new Model class object that ModelNew (which captured the old
    # class at compile time) no longer inherits from, breaking super() calls.
    if "Model" not in backend.context:
        try:
