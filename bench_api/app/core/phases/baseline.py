...
from app.core.phases.common import (
    EvalStage, set_eval_stage, make_timing, load_reference_code, summarize_elapsed_times,
)
...
def run_baseline_phase(
    backend, function: str, hardware: str, compute_capability: str,
    ...
    try:
        ...
        elapsed_times = run_performance(
            backend, 'Model', num_trials=num_trials, num_warmup=num_warmup, torch_compile=torch_compile,
        )
...
def compute_baseline(
    ...
    try:
        result = run_baseline_phase(
            ...
            torch_compile=torch_compile, num_trials=num_trials, num_warmup=num_warmup,
        )
