...
KERNELBENCH_PATH = Path(
    os.getenv("KERNELBENCH_PATH", str(BASE_DIR.parent / "KernelBench"))
)
...
# MLflow settings — disabled by default; set ENABLE_MLFLOW=true to activate.
MLFLOW_TRACKING_URI = os.getenv("MLFLOW_TRACKING_URI", "http://127.0.0.1:5000")
MLFLOW_EXPERIMENT_NAME = os.getenv("MLFLOW_EXPERIMENT_NAME", "KernelBench")
...
# Each dir should contain category subdirs with .py reference files:
#   - directly: {bench_dir}/{category}/{func}.py  (KernelBench-style)
#   - or under reference/: {bench_dir}/reference/{category}/{func}.py
# Defaults to the sibling KernelBench/ directory.
_default_bench_dirs = ",".join(
    str(p)
    for p in [
        BASE_DIR.parent / "KernelBench",
    ]
)
