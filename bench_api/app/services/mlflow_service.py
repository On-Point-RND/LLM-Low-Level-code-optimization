...
def _ensure_mlflow_initialized():
    ...
    try:
        ...
        if parsed.hostname:
            ...
            if result != 0:
                logger.warning(
                    f"MLflow server at {MLFLOW_TRACKING_URI} is not reachable, MLflow logging may fail"
                )
    ...
    try:
        ...
        if experiment is None:
            ...
            try:
                ...
                logger.warning(
                    f"Could not set experiment as active (non-critical): {e}"
                )
        else:
            experiment_id = experiment.experiment_id
            if (
                hasattr(experiment, "lifecycle_stage")
                and experiment.lifecycle_stage == "deleted"
            ):
                logger.warning(
                    f"Experiment '{experiment_name}' is deleted. Attempting to restore..."
                )
                try:
                    client = MlflowClient()
                    client.restore_experiment(experiment_id)
                    logger.info(
                        f"Restored MLflow experiment: {experiment_name} (id: {experiment_id})"
                    )
                    try:
                        mlflow.set_experiment(experiment_name)
                    except Exception as e:
                        logger.warning(
                            f"Could not set restored experiment as active (non-critical): {e}"
                        )
                except Exception as restore_error:
                    logger.error(
                        f"Failed to restore deleted experiment '{experiment_name}': {restore_error}"
                    )
                    logger.info(
                        f"Creating new experiment with name '{experiment_name}_new'"
                    )
                    new_name = f"{experiment_name}_new"
                    experiment_id = mlflow.create_experiment(new_name)
                    logger.info(
                        f"Created new MLflow experiment: {new_name} (id: {experiment_id})"
                    )
            else:
                logger.debug(
...
def log_evaluation_result(
    ...
    if not experiment_name:
        if MLFLOW_EXPERIMENT_NAME:
            ...
            logger.warning(
                "No experiment name provided and MLFLOW_EXPERIMENT_NAME not set"
            )
            return None
    else:
    ...
    try:
        ...
        with mlflow.start_run(
            experiment_id=experiment_id, run_name=final_run_name
        ) as run:
            # Log parameters
            ...
            if performance:
                ...
                mlflow.log_metric(
                    "performance_num_trials", performance.get("num_trials", 0)
                )
            ...
            logger.info(
                f"Logged evaluation to MLflow: experiment={target_experiment_name}, run_id={run.info.run_id}, run_name={final_run_name}"
            )
            ...
            safe_experiment_name = (
                target_experiment_name.replace("/", "_")
                .replace("\\", "_")
                .replace(" ", "_")
            )
            ...
            if target_experiment_name not in _experiment_log_files:
                ...
                _experiment_log_files[target_experiment_name] = (
                    LOGS_DIR / f"{safe_experiment_name}_{timestamp_str}.jsonl"
                )
...
def log_error_to_run(run_id: str, error_text: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Failed to log error artifact for run {run_id}: {artifact_e}"
            )
...
def log_baseline_result(
    ...
    if experiment_id is None:
        logger.error(
            f"Failed to get or create experiment '{target_experiment_name}' for baseline"
        )
        return None
    ...
    try:
        ...
        with mlflow.start_run(experiment_id=experiment_id, run_name=run_name) as run:
            ...
            logger.info(
                f"Logged baseline to MLflow: experiment={target_experiment_name}, run_id={run.info.run_id}, run_name={run_name}"
            )
