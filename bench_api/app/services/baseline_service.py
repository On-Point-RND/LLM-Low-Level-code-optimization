...
from app.config import BASELINES_DIR
...
def _build_cache_key(
    ...
    device_id: Optional[int] = None,  # kept for API compat, intentionally ignored
) -> tuple:
    """Returns (dims_key, cache_key).
    device_id is excluded from the key: the same hardware produces the same baseline
    regardless of which physical device index is used.
    """
...
def get_all_baselines(language: str, torch_compile: bool = False) -> Dict[str, Any]:
    ...
    for func_name in get_dataset().keys():
