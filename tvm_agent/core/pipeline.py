...
def _save_llm_tir_step(res_dir, step_data):
    ...
    # Optional: code after all preprocessing (metadata injection, import stripping)
    parsed_code = step_data.get("code_for_parse")
    if parsed_code is not None:
        with open(os.path.join(step_dir, "tir_code_parsed.py"), "w") as f:
            f.write(parsed_code)
...
def process_baseline(baseline_path, config_path, kernelbench_root, res_root, gpu_id=0):
    ...
    if transform_tir_mode == "llm_transform_tir":
        llm_tir_strategy = str(config.get("llm_tir_strategy", "feedback")).strip().lower()
        if llm_tir_strategy not in ("feedback", "sampling"):
            print(
                f"  [7/10] Unknown llm_tir_strategy={config.get('llm_tir_strategy')!r}; using 'feedback'.",
                flush=True,
            )
            llm_tir_strategy = "feedback"
        metrics["llm_tir_strategy"] = llm_tir_strategy
        if llm_tir_strategy == "sampling":
            print("  [7/10] Applying LLM transformation (TIR / TVMScript, sampling / single-round pipeline)...")
        else:
            print("  [7/10] Applying LLM transformation (TIR / TVMScript, multi-step feedback)...")
        max_steps = config.get("llm_tir_max_steps", 3)
        baseline_mean_ms = _load_baseline_mean_ms(res_root, model_path_rel)
        reference_python_code = ""
        with open(baseline_path, "r", encoding="utf-8") as f:
            reference_python_code = f.read()
        llm_logs_dir = os.path.join(res_dir, "llm_logs_tir") if llm_tir_strategy == "sampling" else None
