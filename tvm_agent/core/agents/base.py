...
def call_openrouter(
    ...
    connection_refused_marker = "urlopen error [Errno 111] Connection refused"
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            err_text = str(err)
            is_connection_refused = connection_refused_marker in err_text
            has_attempts_left = attempt < max_retries
            if not is_connection_refused or not has_attempts_left:
                raise
